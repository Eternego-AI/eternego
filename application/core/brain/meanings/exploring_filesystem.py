"""Meaning — exploring and working with files and directories."""
...
def intention(persona: Persona) -> str:
    return f"{persona.name} wants to read, write, create, copy, or delete files and directories"


def prompt(persona: Persona) -> str:
    workspace = str(paths.workspace(persona.id))
    home = str(paths.home(persona.id))
    return (
