...
# ── Needs acknowledgement / decision / conclusion ────────────────────────────────

async def test_needs_acknowledgement_when_last_signal_is_heard():
    def isolated():
        import os
        import tempfile
        from datetime import datetime
        ...
        mind.realize(s, "greeting")
        mind.understand(mind.perceptions[0], TestMeaning(p))
        assert len(mind.needs_acknowledgement) == 1

    code, error = await on_separate_process_async(isolated)
    assert code == 0, error


async def test_needs_acknowledgement_empty_when_no_reply():
    def isolated():
        import os
        import tempfile
        from datetime import datetime
        ...
        assert len(mind.needs_acknowledgement) == 0
...
async def test_accept_reality_clears_open_thoughts_keeps_concluded():
    def isolated():
        import os
        import tempfile
        from datetime import datetime
        ...
        # Create an open thought
        s1 = Signal(id="s1", event=SignalEvent.heard, content="open task", created_at=datetime(2026, 3, 15, 10, 0))
        mind.trigger(s1)
        mind.realize(s1, "open topic")
        mind.understand(mind.perceptions[0], TestMeaning(p))

        # Create a concluded thought (recap present)
        s2 = Signal(id="s2", event=SignalEvent.heard, content="done task", created_at=datetime(2026, 3, 15, 10, 1))
        mind.trigger(s2)
        mind.realize(s2, "done topic")
        thought2 = mind.understand(mind.perceptions[1], TestMeaning(p))
        mind.answer(thought2, "all done", SignalEvent.recap)

        assert len(mind.intentions) == 2

        mind.accept_reality([], [], {}, {})

        # Only the concluded thought survives
        assert len(mind.intentions) == 1
        assert mind.intentions[0].perception.impression == "done topic"

    code, error = await on_separate_process_async(isolated)
    assert code == 0, error


async def test_accept_reality_routes_signals_to_new_perceptions():
    def isolated():
        import os
        import tempfile
        from datetime import datetime
        ...
        signal_index = {"s1": s1}
        new_perceptions = [{"impression": "greeting", "thread": ["s1"]}]

        mind.accept_reality(new_perceptions, [], signal_index, {})

        assert len(mind.perceptions) == 1
        assert mind.perceptions[0].impression == "greeting"
        assert s1 in mind.perceptions[0].thread

    code, error = await on_separate_process_async(isolated)
    assert code == 0, error


async def test_accept_reality_creates_thoughts_with_meanings():
    def isolated():
        import os
        import tempfile
        from datetime import datetime
        ...
        meaning = TestMeaning(p)
        mind = Memory(p, [meaning])
        s1 = Signal(id="s1", event=SignalEvent.heard, content="hello", created_at=datetime(2026, 3, 15, 10, 0))
        mind.trigger(s1)

        signal_index = {"s1": s1}
        meaning_index = {"Test": meaning}
        new_perceptions = [{"impression": "greeting", "thread": ["s1"]}]
        new_thoughts = [{"perception": "greeting", "meaning": "Test", "priority": 5}]

        mind.accept_reality(new_perceptions, new_thoughts, signal_index, meaning_index)

        assert len(mind.intentions) == 1
        assert mind.intentions[0].meaning.name == "Test"
        assert mind.intentions[0].priority == 5

    code, error = await on_separate_process_async(isolated)
    assert code == 0, error


# ── perception_by_impression ────────────────────────────────────────────────

async def test_perception_by_impression_returns_matching():
    def isolated():
        import os
        import tempfile
        from datetime import datetime
        ...
        result = mind.perception_by_impression("greeting")
        assert result is not None
        assert result.impression == "greeting"

    code, error = await on_separate_process_async(isolated)
    assert code == 0, error


async def test_perception_by_impression_returns_none_for_unknown():
    def isolated():
        import os
        import tempfile
        from application.core.brain.data import Meaning
        ...
        mind = Memory(p, [TestMeaning(p)])
        result = mind.perception_by_impression("nonexistent")
