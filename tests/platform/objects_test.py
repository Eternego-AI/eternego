...
async def test_safe_masks_sensitive_fields():
...
async def test_safe_excludes_hidden_fields():
    user = User(name="alice", password="secret123", bus="internal")
    result = safe(user)
    assert "bus" not in result


async def test_safe_shows_empty_string_for_none_sensitive():
    user = User(name="alice", password=None, bus=None)
    result = safe(user)
    assert result["password"] == ""


async def test_json_includes_sensitive_values():
    user = User(name="alice", password="secret123", bus="internal")
    result = json(user)
    assert result["password"] == "secret123"


async def test_json_excludes_hidden_fields():
    user = User(name="alice", password="secret123", bus="internal")
    result = json(user)
    assert "bus" not in result


async def test_safe_handles_nested_data():
    @dataclass
    class Wrapper(Data):
        user: User
...
async def test_safe_handles_lists_and_dicts():
    result = safe({"items": [1, "two", None, True]})
    assert result == {"items": [1, "two", None, True]}


async def test_safe_converts_path_to_string():
...
async def test_are_equal_compares_by_serialized_form():
...
async def test_are_equal_detects_differences():
