...
def test_it_hashes_strings_consistently():
    a = sha256("hello")
    b = sha256("hello")
    assert a == b


def test_it_hashes_different_inputs_differently():
    assert sha256("hello") != sha256("world")


def test_it_hashes_bytes_directly():
    result = sha256(b"hello")
    assert isinstance(result, str)
    assert len(result) == 64


def test_it_generates_short_unique_ids():
    result = generate_unique_id("some content")
    assert len(result) == 6
    assert result == sha256("some content")[:6]


def test_it_generates_different_ids_for_different_content():
    assert generate_unique_id("a") != generate_unique_id("b")


def test_it_encrypts_and_decrypts_roundtrip():
...
def test_it_derives_consistent_keys():
...
def test_it_derives_different_keys_for_different_secrets():
    salt = b"fixed-salt-value"
